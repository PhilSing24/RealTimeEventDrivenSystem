...
tmux split-window -v -t $SESSION
tmux send-keys -t $SESSION "sleep 4 && cd ~/binance_feed_handler && q kdb/tel.q" C-m

tmux select-pane -t $SESSION:0.0
tmux split-window -v -t $SESSION
tmux send-keys -t $SESSION "sleep 5 && cd ~/binance_feed_handler && ./build/binance_feed_handler" C-m

tmux select-layout -t $SESSION tiled
echo "TP:5010 RDB:5011 RTE:5012 TEL:5013 FH"
